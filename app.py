import streamlit as st
import google.generativeai as genai
import json
import time
import pandas as pd
import matplotlib.pyplot as plt

# --- CONFIGURATION STYLE & PAGE ---
st.set_page_config(page_title="Gemini Exam Sim", page_icon="🧠", layout="wide")

# CSS pour le style "Gemini" et le minuteur d'alerte
st.markdown("""
<style>
    .stApp {
        background-color: #0E1117;
        color: #FAFAFA;
    }
    .stButton>button {
        background-color: #2e4a9e;
        color: white;
        border-radius: 8px;
        border: none;
    }
    .stRadio {
        background-color: #161b22;
        padding: 20px;
        border-radius: 10px;
    }
    .timer-box {
        font-size: 24px;
        font-weight: bold;
        padding: 10px;
        border-radius: 5px;
        text-align: center;
        margin-bottom: 20px;
    }
    .timer-normal {
        color: #00ff00;
        border: 1px solid #00ff00;
    }
    .timer-alert {
        color: #ff0000;
        border: 2px solid #ff0000;
        animation: blinker 1s linear infinite;
    }
    @keyframes blinker {
        50% { opacity: 0; }
    }
</style>
""", unsafe_allow_html=True)

# --- FONCTIONS UTILITAIRES ---

def extract_text_from_file(uploaded_file):
    """Extrait le texte d'un fichier uploadé (txt pour l'instant, extensible)"""
    if uploaded_file is None:
        return ""
    try:
        stringio = uploaded_file.getvalue().decode("utf-8")
        return stringio
    except Exception as e:
        return f"Erreur de lecture : {e}"

def generate_quiz_data(topic_text, num_questions, include_graphs=True):
    """Demande à Gemini de générer un JSON structuré pour le quiz"""
    model = genai.GenerativeModel('gemini-1.5-flash')
    
    graph_instruction = ""
    if include_graphs:
        graph_instruction = """
        Pour 30% des questions, inclus un champ 'graph_data' contenant des points (x, y) 
        pour tracer une courbe économique simple (ex: offre/demande, coût marginal).
        Structure: {"x": [1,2,3], "y": [10,20,30], "xlabel": "Qté", "ylabel": "Prix", "title": "..."}
        La question doit nécessiter la lecture de ce graphe.
        """

    prompt = f"""
    Tu es un créateur d'examen universitaire expert.
    Basé sur le texte suivant : {topic_text[:15000]} (tronqué si trop long)...
    
    Génère un examen de {num_questions} questions au format JSON STRICT.
    Il ne doit y avoir QUE du JSON, pas de texte avant ou après.
    
    Format attendu :
    [
        {{
            "id": 1,
            "question": "L'énoncé de la question...",
            "options": {{"A": "...", "B": "...", "C": "...", "D": "...", "E": "..."}},
            "correct_answer": "A",
            "explanation": "L'explication détaillée de la réponse...",
            "graph_data": nullOrObject
        }}
    ]
    
    {graph_instruction}
    """
    
    try:
        response = model.generate_content(prompt)
        # Nettoyage basique au cas où Gemini mettrait des ```json
        clean_json = response.text.replace("```json", "").replace("```", "").strip()
        return json.loads(clean_json)
    except Exception as e:
        st.error(f"Erreur de génération IA : {e}")
        return []

def render_graph(data):
    """Affiche un graphique matplotlib basé sur les données JSON"""
    if not data:
        return
    fig, ax = plt.subplots(figsize=(6, 4))
    # Style sombre pour aller avec l'interface
    plt.style.use('dark_background')
    ax.plot(data['x'], data['y'], marker='o', linestyle='-', color='#4fa8d1')
    ax.set_xlabel(data.get('xlabel', 'X'))
    ax.set_ylabel(data.get('ylabel', 'Y'))
    ax.set_title(data.get('title', 'Graphique'))
    ax.grid(True, alpha=0.3)
    st.pyplot(fig)

# --- INTERFACE PRINCIPALE ---

def main():
    st.title("🎓 Gemini Exam Simulator")

    # --- SIDEBAR : PARAMÈTRES ---
    with st.sidebar:
        st.header("⚙️ Configuration")
        
        api_key = st.text_input("Clé API Gemini", type="password")
        if api_key:
            genai.configure(api_key=api_key)

        st.subheader("📂 Documents de cours")
        uploaded_file = st.file_uploader("Charge ton cours (.txt)", type=['txt'])
        course_text = extract_text_from_file(uploaded_file)
        
        st.divider()
        
        st.subheader("⏱️ Paramètres du Test")
        mode = st.radio("Type d'épreuve", ["Entraînement Chapitre", "Examen Final (Simulé)"])
        
        if mode == "Entraînement Chapitre":
            num_q = st.slider("Nombre de questions", 5, 15, 8)
            time_limit = st.slider("Temps (minutes)", 10, 60, 30)
        else:
            num_q = st.number_input("Nombre de questions", min_value=10, max_value=100, value=70)
            time_limit = st.number_input("Temps (heures)", 1, 4, 3) * 60 # Conversion en minutes

        if st.button("🚀 LANCER L'EXAMEN", type="primary"):
            if not api_key or not course_text:
                st.warning("Il manque la clé API ou le fichier de cours !")
            else:
                with st.spinner("Génération de l'examen et des graphiques..."):
                    quiz_data = generate_quiz_data(course_text, num_q, include_graphs=True)
                    if quiz_data:
                        st.session_state.quiz_data = quiz_data
                        st.session_state.current_index = 0
                        st.session_state.score = 0
                        st.session_state.start_time = time.time()
                        st.session_state.total_time_seconds = time_limit * 60
                        st.session_state.user_answers = {}
                        st.session_state.quiz_active = True
                        st.rerun()

    # --- LOGIQUE DU QUIZ ---
    if 'quiz_active' in st.session_state and st.session_state.quiz_active:
        
        # 1. GESTION DU TEMPS
        elapsed = time.time() - st.session_state.start_time
        remaining = st.session_state.total_time_seconds - elapsed
        
        # Formatage du temps
        mins, secs = divmod(int(remaining), 60)
        timer_text = f"{mins:02d}:{secs:02d}"
        
        # Alerte visuelle
        timer_class = "timer-normal"
        if remaining < 0:
            timer_text = f"RETARD: {timer_text}"
            timer_class = "timer-alert"
        elif remaining < 600: # Moins de 10 minutes
            timer_class = "timer-alert"
            
        st.markdown(f'<div class="timer-box {timer_class}">⏳ {timer_text}</div>', unsafe_allow_html=True)

        # 2. AFFICHAGE QUESTION
        questions = st.session_state.quiz_data
        idx = st.session_state.current_index
        
        if idx < len(questions):
            q_data = questions[idx]
            
            # Barre de progression
            st.progress((idx) / len(questions), text=f"Question {idx + 1}/{len(questions)}")
            
            st.markdown(f"### {q_data['question']}")
            
            # Affichage Graphique si présent
            if q_data.get('graph_data'):
                st.info("📊 Analyse le graphique ci-dessous pour répondre :")
                render_graph(q_data['graph_data'])
            
            # Choix QCM
            options = q_data['options']
            choice = st.radio(
                "Choisis la meilleure réponse :",
                list(options.keys()),
                format_func=lambda x: f"{x}) {options[x]}",
                key=f"q_{idx}"
            )
            
            # Bouton Valider
            if st.button("Valider et Suivant ➡️"):
                # Enregistrement réponse
                st.session_state.user_answers[idx] = {
                    "user": choice,
                    "correct": q_data['correct_answer'],
                    "explanation": q_data['explanation'],
                    "time_spent": elapsed # Timestamp brut, on affinera à la fin
                }
                
                # Calcul score immédiat (invisible pour l'utilisateur en mode exam)
                if choice == q_data['correct_answer']:
                    st.session_state.score += 1
                
                st.session_state.current_index += 1
                st.rerun()
                
        else:
            # 3. ÉCRAN DE RÉSULTATS
            st.session_state.quiz_active = False
            st.balloons()
            
            final_score = st.session_state.score
            total = len(questions)
            note_sur_10 = round((final_score / total) * 10, 1)
            
            st.markdown(f"""
            # 🏁 Examen Terminé !
            ## Note Finale : {note_sur_10}/10
            """)
            
            # Détails et Corrections
            with st.expander("Voir le corrigé détaillé", expanded=True):
                for i, q in enumerate(questions):
                    ans = st.session_state.user_answers.get(i)
                    is_correct = ans['user'] == ans['correct']
                    color = "green" if is_correct else "red"
                    
                    st.markdown(f"**Q{i+1}: {q['question']}**")
                    st.markdown(f":{color}[Ta réponse: {ans['user']} | Bonne réponse: {ans['correct']}]")
                    st.info(f"💡 Explication : {ans['explanation']}")
                    st.divider()

            if st.button("Recommencer un test"):
                del st.session_state.quiz_active
                st.rerun()

    else:
        st.info("👈 Configure ton examen dans la barre latérale et charge un fichier pour commencer.")
        st.markdown("""
        ### ✨ Fonctionnalités actives :
        * **Multi-fichiers** : Charge tes propres cours.
        * **Graphiques IA** : Certains problèmes génèrent des courbes visuelles.
        * **Minuteur Exam** : Alerte rouge si < 10 mins.
        * **Notation précise** : Calcul automatique sur 10.
        """)

if __name__ == "__main__":
    main()
